  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Relatórios - Auditoria</h1>
          <p className="text-muted-foreground mt-1 max-w-xl">
            Consulte e acompanhe as auditorias realizadas nos atendimentos. Use os filtros para analisar eventos
            por período, status e agente.
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="outline" size="sm" onClick={() => setStatusFilter("ok")}>
            <Badge className="mr-1 bg-green-100 text-green-800" variant="outline">OK</Badge>
            Aprovados
          </Button>
          <Button variant="outline" size="sm" onClick={() => setStatusFilter("erro")}>
            <Badge className="mr-1 bg-red-100 text-red-700" variant="outline">Erro</Badge>
            Erros
          </Button>
          <Button variant="outline" size="sm" onClick={() => setStatusFilter("pendente")}>
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={() => {/* popup date range picker futuro*/}}
          >
          ...
          {/* (simples, ideal abrir um calendar em popup aqui) */}
        </div>
      </div>

      {/* Cards de resumo */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {MOCK_RESUMO.map((item, i) => (
          <Card key={item.label}>
